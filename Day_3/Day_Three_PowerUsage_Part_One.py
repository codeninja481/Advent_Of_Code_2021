...
powerUsage = open("Day_Three_PowerUsage_Input_1.txt").read().split('\n')
#powerUsage = open("Day_Three_PowerUsage_Input.txt").read().split('\n')

def binary2Decimal(n):
    return int(n,2)

def getGamaRate():
    for idx, val in enumerate(powerUsage):
